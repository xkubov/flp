...
-- Cv: DOPLŇTE PRAVÉ STRANY DEFINIČNÍCH KLAUSULÍ
step (TMConf _ (Tape x (l:tpl) tpr))
             (Trans _ _ q ALeft)      = undefined  -- DOPLNIT
step (TMConf _ (Tape x tpl (r:tpr)))
             (Trans _ _ q ARight)     = undefined  -- DOPLNIT
step (TMConf _ (Tape _ tpl tpr))
             (Trans _ _ q (AWrite w)) = undefined  -- DOPLNIT
step (TMConf _ (Tape _ [] _))
             (Trans _ _ _ ALeft)      = undefined  -- DOPLNIT
step (TMConf _ (Tape _ _ []))
             _                        = undefined  -- DOPLNIT
